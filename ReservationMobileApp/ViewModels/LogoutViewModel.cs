...
namespace ReservationMobileApp.ViewModels
{
    public class LogoutViewModel : BaseViewModel
    {
        ...
        {   //instant command redirect to the specific page 
            await Application.Current.MainPage.Navigation.PushModalAsync(new CartView());
        }

        private async Task LogoutUserAsync()
        {   //once the user confirm to logout cart and prefences we set will be deleted and push to login page 
            var cis = new CartItemService();
            cis.RemoveItemsFromCart();
            Preferences.Remove("Username");
            await Application.Current.MainPage.Navigation.PushModalAsync(new TabbedLoginPage());
